...
public class PlayerListener implements Listener {
    ...
    public void playerPostLogin(final PostLoginEvent e) {
        String ip = e.getPlayer().getSocketAddress().toString();
            ...
            geSuit.proxy.getScheduler().schedule(geSuit.getInstance(), () -> {
                ...
                if (ConfigManager.bans.GeoIP.ShowOnLogin) {
                    geSuit.proxy.getScheduler().runAsync(geSuit.getInstance(),
                            () -> {
                        SocketAddress socket = e.getPlayer().getSocketAddress();
                        if (socket instanceof InetSocketAddress) {
                            String location = GeoIPManager.lookup(
                                    ((InetSocketAddress) socket).getAddress());
                            String msg;
                            if (location != null) {
                                 msg = ConfigManager.messages.PLAYER_GEOIP.
                                        replace("{player}", p.getName()).
